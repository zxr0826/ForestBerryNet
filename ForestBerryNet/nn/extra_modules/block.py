...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C3k2_Faster', 'C3k2_ODConv', 'Partial_conv3', 'C3k2_Faster_EMA', 'C3k2_DBB',
           ...
           'C3k2_AdditiveBlock', 'C3k2_AdditiveBlock_CGLU', 'HCBR', 'EUCB', 'C3k2_MSMHSA_CGLU', 'CSP_PMSFA', 'C3k2_MogaBlock', 'C3k2_SHSA', 'C3k2_SHSA_CGLU', 'C3k2_SMAFB', 'C3k2_SMAFB_CGLU',
...
class ConvPlaceholder(nn.Module):
    def __init__(self, dim):
        super().__init__()
        # 使用一个3x3的深度可分离卷积来模拟局部特征提取，同时保持轻量
        # 这比标准卷积更公平，因为它更接近轻量化设计的初衷
        self.conv = nn.Sequential(
    ...
    def forward(self, x):
        # 输入x的维度是 [B, C, H, W]，直接通过卷积处理
        # 不需要像LocalGlobalAttention那样进行复杂的permute和reshape
        return self.conv(x)
...
class HAFB(nn.Module):
    ...
    def __init__(self, inc, ouc, group=False):
        ...
        #self.lgb1_local = ConvPlaceholder(hidc)
        #self.lgb1_global = ConvPlaceholder(hidc)
        #self.lgb2_local = ConvPlaceholder(hidc)
        #self.lgb2_global = ConvPlaceholder(hidc)
