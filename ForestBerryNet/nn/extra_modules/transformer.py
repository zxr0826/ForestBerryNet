...
__all__ = ['AIFI_RepBN', 'C2BRA', 'C2CGA', 'C2DA', 'C2DPB', 'C2Pola', 'C2TSSA', 'C2ASSA']
...
class C2ASSA(C2PSA):
    def __init__(self, c1, c2, n=1, e=0.5):
        ...
        self.m = nn.Sequential(*(AdaptiveSparseSA(self.c, 8, True) for _ in range(n)))
    
    def forward(self, x):
        """Processes the input tensor 'x' through a series of PSA blocks and returns the transformed tensor."""
        a, b = self.cv1(x).split((self.c, self.c), dim=1)
        BS, C, H, W = b.size()
        b = self.m(b).permute(0, 2, 1).view([-1, C, H, W]).contiguous()
        return self.cv2(torch.cat((a, b), 1))
