...
from ForestBerryNet.utils.tal import dist2bbox, make_anchors, dist2rbox
# from ForestBerryNet.utils.ops import nmsfree_postprocess
...
class Detect_Efficient(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):  # detection layer
        ...
        #self.stem = nn.ModuleList(nn.Sequential(Conv(x, x, 3), Conv(x, x, 3)) for x in ch) # one 3x3 Conv, one 3x3 Conv
        #self.stem = nn.ModuleList(nn.Sequential(DWConv(x, x, 3), Conv(x, x, 1)) for x in ch) # one 3x3 DWConv, one 1x1 Conv
        #self.stem = nn.ModuleList(nn.Sequential(Conv(x, x, 3), Conv(x, x, 1)) for x in ch) # one 3x3 Conv, one 1x1 Conv
        # self.stem = nn.ModuleList(nn.Sequential(RepConv(x, x, 3), Conv(x, x, 1)) for x in ch) # one 3x3 RepConv, one 1x1 Conv
...
class Detect_SEAM(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.export and self.format in ('tflite', 'edgetpu'):
            ...
            # https://github.com/ForestBerryNet/yolov5/blob/0c8de3fca4a702f8ff5c435e67f378d1fce70243/models/tf.py#L307-L309
            # See this PR for details: https://github.com/ForestBerryNet/ForestBerryNet/pull/1695
...
class Detect_LSCD(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.export and self.format in ("tflite", "edgetpu"):
            ...
            # See https://github.com/ForestBerryNet/ForestBerryNet/issues/7371
            img_h = shape[2]
            img_w = shape[3]
