...
HELP_MSG = """
    Examples for running ForestBerryNet:

    1. Install the ForestBerryNet package:

        pip install ForestBerryNet
    ...
    2. Use the Python SDK:
        ...
        results = model("https://ForestBerryNet.com/images/bus.jpg")  # predict on an image
        success = model.export(format="onnx")  # export the model to ONNX format

    3. Use the command line interface (CLI):

        ForestBerryNet 'yolo' CLI commands use the following syntax:
            ...
            Where   TASK (optional) is one of [detect, segment, classify, pose, obb]
                    ...
                    ARGS (optional) are any number of custom "arg=value" pairs like "imgsz=320" that override defaults.
                        See all ARGS at https://docs.ForestBerryNet.com/usage/cfg or with "yolo cfg"
    ...
    Community: https://community.ForestBerryNet.com
    GitHub: https://github.com/ForestBerryNet/ForestBerryNet
    """
...
class TQDM(tqdm_original):
    ...
    Examples:
        >>> from ForestBerryNet.utils import TQDM
    ...
    def __init__(self, *args, **kwargs):
        ...
        This class extends the original tqdm class to provide customized behavior for ForestBerryNet projects.
...
class IterableSimpleNamespace(SimpleNamespace):
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(
            f"""
            '{name}' object has no attribute '{attr}'. This may be caused by a modified or out of date ForestBerryNet
            'default.yaml' file.\nPlease update your code with 'pip install -U ForestBerryNet' and if necessary replace
            {DEFAULT_CFG_PATH} with the latest version from
            https://github.com/ForestBerryNet/ForestBerryNet/blob/main/ForestBerryNet/cfg/default.yaml
            """
        )
...
def set_logging(name="LOGGING_NAME", verbose=True):
    ...
    This function configures logging for the ForestBerryNet library, setting the appropriate logging level and
    ...
    Examples:
        >>> set_logging(name="ForestBerryNet", verbose=True)
        >>> logger = logging.getLogger("ForestBerryNet")
...
class ThreadingLocked:
    ...
    Example:
        ```python
        from ForestBerryNet.utils import ThreadingLocked
...
def get_user_config_dir(sub_dir="ForestBerryNet"):
    """
...
USER_CONFIG_DIR = Path(os.getenv("YOLO_CONFIG_DIR") or get_user_config_dir())  # ForestBerryNet settings dir
...
class TryExcept(contextlib.ContextDecorator):
    """
    ForestBerryNet TryExcept class. Use as @TryExcept() decorator or 'with TryExcept():' context manager.
...
class SettingsManager(JSONDict):
    """
    SettingsManager class for managing and persisting ForestBerryNet settings.
    ...
    def __init__(self, file=SETTINGS_FILE, version="0.0.6"):
        ...
        from ForestBerryNet.utils.torch_utils import torch_distributed_zero_first
        ...
        self.help_msg = (
            f"\nView ForestBerryNet Settings with 'yolo settings' or at '{self.file}'"
            "\nUpdate Settings with 'yolo settings key=value', i.e. 'yolo settings runs_dir=path/to/dir'. "
            "For help see https://docs.ForestBerryNet.com/quickstart/#ForestBerryNet-settings."
        )
        ...
        with torch_distributed_zero_first(RANK):
            ...
            if not self.file.exists() or not self:  # Check if file doesn't exist or is empty
                LOGGER.info(f"Creating new ForestBerryNet Settings v{version} file ✅ {self.help_msg}")
                self.reset()
    ...
    def _validate_settings(self):
        ...
        if not (correct_keys and correct_types and correct_version):
            LOGGER.warning(
                "WARNING ⚠️ ForestBerryNet settings reset to default values. This may be due to a possible problem "
                f"with your settings or a recent ForestBerryNet package update. {self.help_msg}"
            )
            self.reset()

        if self.get("datasets_dir") == self.get("runs_dir"):
            LOGGER.warning(
                f"WARNING ⚠️ ForestBerryNet setting 'datasets_dir: {self.get('datasets_dir')}' "
    ...
    def update(self, *args, **kwargs):
        ...
        for k, v in kwargs.items():
            if k not in self.defaults:
                raise KeyError(f"No ForestBerryNet setting '{k}'. {self.help_msg}")
            t = type(self.defaults[k])
            if not isinstance(v, t):
                raise TypeError(f"ForestBerryNet setting '{k}' must be of type '{t}', not '{type(v)}'. {self.help_msg}")
        super().update(*args, **kwargs)
...
def vscode_msg(ext="ForestBerryNet.ForestBerryNet-snippets") -> str:
    """Display a message to install ForestBerryNet-Snippets for VS Code if not already installed."""
    path = (USER_CONFIG_DIR.parents[2] if WINDOWS else USER_CONFIG_DIR.parents[1]) / ".vscode/extensions"
    obs_file = path / ".obsolete"  # file tracks uninstalled extensions, while source directory remains
    installed = any(path.glob(f"{ext}*")) and ext not in (obs_file.read_text("utf-8") if obs_file.exists() else "")
    url = "https://docs.ForestBerryNet.com/integrations/vscode"
    return "" if installed else f"{colorstr('VS Code:')} view ForestBerryNet VS Code Extension ⚡ at {url}"


# Run below code on utils init ------------------------------------------------------------------------------------

# Check first-install steps
PREFIX = colorstr("ForestBerryNet: ")
...
from ForestBerryNet.utils.patches import imread, imshow, imwrite, torch_load, torch_save
