...
from ForestBerryNet.data.utils import FORMATS_HELP_MSG, IMG_FORMATS, VID_FORMATS
from ForestBerryNet.utils import IS_COLAB, IS_KAGGLE, LOGGER, ops
...
class LoadImagesAndVideos:
    ...
    def __init__(self, path, batch=1, vid_stride=1):
        ...
        for p in sorted(path) if isinstance(path, (list, tuple)) else [path]:
            a = str(Path(p).absolute())  # do not use .resolve() https://github.com/ForestBerryNet/ForestBerryNet/issues/2912
            if "*" in a:
...
def autocast_list(source):
    ...
    for im in source:
        if isinstance(im, (str, Path)):  # filename or uri
            ...
            raise TypeError(
                f"type {type(im).__name__} is not a supported ForestBerryNet prediction source type. \n"
                f"See https://docs.ForestBerryNet.com/modes/predict for supported source types."
            )
